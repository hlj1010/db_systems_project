...
\node[entity] (doc) {Doctor};
	...
	\node[attribute] (dadress) [below left=2.5cm  of doc,xshift=0.5cm] {address} edge (doc);
...
% patient entity
\node[entity] (pat) [right=1cm of treat,xshift=6cm] {Patient} edge [total] (treat);
	...
	% address attribute composite
		\node[attribute] (pad) [below left=2cm  of pat,xshift=1cm,yshift=0.2cm] {address} edge (pat);
		...
		\node[attribute] (pzip) [above =0.3 of pad] {zip} edge (pad);
	
	% phone attribute composite
	\node[attribute] (pphone) [below=0.3cm  of pat,xshift=01cm] {phone} edge (pat);
		\node[attribute] (ppphone) [below=1cm  of pphone,xshift=2cm] {work} edge (pphone);
		\node[attribute] (pwphone) [below =1.5cm of pphone,yshift=1cm] {cell} edge (pphone);
...
\node[weak entity] (med) [above=3.6cm of pat] {Medical Record} edge [total] (has);	
...
%=======
% illness entity
\node[entity] (ill) [left = 3cm of suf] {illness} edge (suf);
	% name
	\node[attribute] (illnm) [above=1cm  of ill] {\key{name}} edge (ill);
	% symptom attribute 
	\node[multi attribute] (symt) [below=1cm  of ill,xshift=-2cm, draw = black] {symptoms} edge (ill);
...
%>>>>>>> 441fec2ba41ce055e8459fc7965762c43412921b
