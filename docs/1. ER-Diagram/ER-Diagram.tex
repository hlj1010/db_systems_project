...
\node[entity] (doc) {Doctor};
	...
	%id attribute - primary key
  	...
  	\node[attribute] (dstart) [above left=2.5 cm of doc, xshift = 18mm] {start\_date} edge (doc);
	...
	\node[attribute] (dstr) [below=0.7cm of dadress,xshift=0.6cm] {street} edge (dadress);
	...
	\node[attribute] (dzip) [below right =0.3 of dadress] {zipcode} edge (dadress);
...
% patient entity
\node[entity] (pat) [below=4cm of treat] {Patient} edge [total] (treat);
