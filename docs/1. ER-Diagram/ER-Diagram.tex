...
% treats relationship
\node[relationship] (treat) [right=4cm of doc] {Treats} edge (doc);
	
	% start data attribute
	\node[multi attribute] (tnotes) [above =0.5cm of treat,draw = black] {notes} edge (treat);
	\node[derived attribute] (tstart) [below right =0.2cm of treat,draw = black] {start\_date} edge (treat);
	%consultation dates 
...
\node[entity] (pat) [below=2.5cm of treat] {Patient} edge [total] (treat);
	...
	\node[attribute] (pins) [below right=0.6 of pat,yshift = 0.6 cm] {insurance} edge (pat);
		...
		\node[attribute] (pinnum) [above right=0.3cm of pins] {policy\_number} edge (pins);
		\node[attribute] (pinauth) [right=0.3cm of pins,yshift=-0.1cm] {authorization\_number} edge (pins);
	
\node[relationship] (has) [above right=2cm of pat] {has} edge  [->] (pat);

% patient entity
\node[weak entity] (med) [right=3.4cm of treat] {Medical Record} edge [total] (has);	
	...
	\node[attribute] (mbirth) [above =0.5 cm of med] {date of birth} edge (med);
	...
	\node[multi attribute] (mmed) [below = 1.4cm of med,draw=black] {prescribed\_medication} edge (med);
